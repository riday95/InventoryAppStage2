...
public class BookProvider extends ContentProvider {
    ...
    static {
        ...
        // For example, "content://com.example.android.inventoryapp/books/3" matches, but
        // "content://com.example.android.inventoryapp/books" (without a number at the end) doesn't match.
    ...
    private Uri insertBook(Uri uri, ContentValues values) {
        ...
        String nameValue = values.getAsString(BookEntry.COLUMN_PRODUCT_NAME);
        if (nameValue == null) {
            throw new IllegalArgumentException("Book requires a name.");
        }

        // Check that the valid price is provided
        double priceValue = values.getAsDouble(BookEntry.COLUMN_PRODUCT_PRICE);
        if (priceValue < 0) {
            throw new IllegalArgumentException("Book requires a valid price.");
        }

        // Check that the valid quantity is provided
        int quantityValue = values.getAsInteger(BookEntry.COLUMN_PRODUCT_QUANTITY);
        if (quantityValue < 0) {
            throw new IllegalArgumentException("Book requires a valid quantity.");
        }

        // Check that the supplier name is not null
        String supplierNameValue = values.getAsString(BookEntry.COLUMN_PRODUCT_SUPPLIER_NAME);
        ...
        String supplierPhoneNumberValue = values.getAsString(BookEntry.COLUMN_PRODUCT_SUPPLIER_PHONE_NUMBER);
    ...
    private int updateBook(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(BookEntry.COLUMN_PRODUCT_QUANTITY)) {
            double quantityValue = values.getAsDouble(BookEntry.COLUMN_PRODUCT_QUANTITY);
            if (quantityValue < 0) {
